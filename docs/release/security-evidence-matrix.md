...
| `MIG-009` | release-blocker | M7 release gates, staged/installed `win32-x64` evidence, M7j2a provenance, M7j3 renderer evidence, M7k2a listing metadata, exact deterministic `0.2.3` pre-release VSIX, and live `0.2.3` attestation and Marketplace Gallery state | Signing/provenance trust, public-install verification, and previous-stable upgrade/rollback are not complete. |
