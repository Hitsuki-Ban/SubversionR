...
| `SEC-009` | verified | Stdio-only public transport implementation and tests plus M8 I3 private one-operation worker pipes with an explicit Windows inherited-handle allowlist, suspended Job assignment, and no IPC sockets | Preserve the packaged no-socket boundary as remote transports are implemented. |
| `SEC-010` | verified | JSON-RPC frame, auth wait, flood, and timeout tests plus the M8 I3 bounded, versioned, exactly-one-request worker protocol, allowlisted worker-failure tests, and production-supervisor cancellation/setup-timeout/child-crash/trust-revoke cleanup matrix | Add coverage-guided fuzzing before public readiness. |
...
| `SEC-015` | release-blocker | Deterministic `0.2.5` pre-release VSIX/package evidence, exact then-pending candidate contract, candidate/historical provenance separation, live `0.2.5` custom-predicate attestation and Marketplace publication evidence, vulnerability decisions, and native artifact mapping | Final SBOM/NOTICE/legal and vulnerability approval, reproducible or signed source-to-binary provenance, public-install verification, and artifact signing are not complete. |
...
| `MIG-009` | release-blocker | M7 release gates, staged/installed `win32-x64` evidence, M7j2a provenance, M7j3 renderer evidence, M7k2a listing metadata, exact deterministic `0.2.5` pre-release VSIX, and live `0.2.5` attestation and Marketplace Gallery state | Signing/provenance trust, public-install verification, and previous-stable upgrade/rollback are not complete. |
...
| `TST-024` | release-blocker | Installed package/workflow/UI evidence, deterministic `0.2.5` pre-release VSIX tests, M8 I3 packaged-daemon and installed-Extension-Host isolated-worker foundation evidence with local-discovery continuity, zero temp-root residue, same-lane reuse, and subsequent diagnostics, candidate-versus-historical provenance/publication/Beta-G separation tests, successful `0.2.5` attestation and Marketplace publication runs, vulnerability gates, native artifact mapping, and Source Control UI E2E evidence | Public-install verification, previous-stable upgrade/rollback, final vulnerability approval, signed source-to-binary provenance, and artifact signing remain incomplete; I3 evidence does not establish remote transport support. |
