...
- The same harness records `sourceControlUiReadonlyPropertyReportWorkflow`, executing the public repository and resource Properties commands twice for exact repository-session targets, proving each identity reuses one `svn-r-diagnostics` read-only Markdown tab with current content, closes without a dirty document or save prompt, and leaves Source Control projection, status refresh, reconcile, and remote polling activity unchanged. Repository/resource Mergeinfo commands remain registered but hidden from all user-facing menus and the Command Palette.
- The same harness records `sourceControlUiCommitAllWorkflow`, `commitAllRepositoryOracle`, the existing `sourceControlUiCommitSelectedMultiSelectionWorkflow` / `commitSelectedMultiSelectionRepositoryOracle` resource-array contract, the independent `sourceControlUiReviewCommitPromptWorkflow` / `reviewCommitPromptRepositoryOracle`, and `commitPromptRendererCaptures`. This evidence proves an empty Source Control input prompts once before a successful local commit; the source-built SVN CLI oracle requires the new local-file commit revision's `svn:author` to be non-empty and Ordinal-equal to the fixture's initial CLI-created revision author; prompt cancellation leaves bytes, projection, refresh, journal, and message history unchanged; Review & Commit retains its exact selection after cancellation and a real forced commit failure; only reviewed paths are committed; an unselected modified probe remains uncommitted; and the successful retry performs authoritative targeted reconcile. The author oracle is limited to Windows `win32-x64` file-backed commits and does not claim remote authentication behavior.
...
- The installed Source Control UI evidence must include `sourceControlUiReadonlyPropertyReportWorkflow` with deterministic repository/resource report identity, single-tab reuse, read-only Markdown metadata, close-without-save behavior, and unchanged status/reconcile/remote activity counters.
- The installed Source Control UI evidence must prove `sourceControlUiCommitAllWorkflow.assertions.emptyInputPrompted` and `commitAllRepositoryOracle.committedRevisionAuthorMatchedInitialCliRevision = true` with non-empty initial and committed revision authors; preserve `sourceControlUiCommitSelectedMultiSelectionWorkflow.command.argumentShape = resourceStateArray`; and publish independent `sourceControlUiReviewCommitPromptWorkflow` plus `reviewCommitPromptRepositoryOracle` evidence for Review & Commit prompt cancellation without byte/state/projection/journal/history mutation, retained reviewed selection after cancellation and forced failure, exact reviewed-path repository state, an unselected modified probe remaining uncommitted, and targeted reconcile after the successful retry.
...
The published 0.2.4 candidate preserves the `SVN-R` Marketplace identity while packaging the completed Source Control information-architecture, runtime count-policy, working-copy metadata grouping, and installed UI-evidence hardening slices:

- root and extension versions are `0.2.4`; the extension version remains plain `major.minor.patch`, while the GitHub pre-release tag is `v0.2.4-beta.1`;
...
- the released GitHub subject is `subversionr-win32-x64-0.2.4.vsix`, 8,295,021 bytes, SHA256 `880e7937423695ca772436f01e2419498463ebd7cc25ba8a283a135530418249`, from source commit `22e6645067e64f3b403392c4648a8b23bdca1b09` and release ID `354129040`;
