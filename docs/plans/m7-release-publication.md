...
- The same harness records `sourceControlUiCommitAllWorkflow`, the existing `sourceControlUiCommitSelectedMultiSelectionWorkflow` / `commitSelectedMultiSelectionRepositoryOracle` resource-array contract, the independent `sourceControlUiReviewCommitPromptWorkflow` / `reviewCommitPromptRepositoryOracle`, and `commitPromptRendererCaptures`. This evidence proves an empty Source Control input prompts once before a successful local commit; prompt cancellation leaves bytes, projection, refresh, journal, and message history unchanged; Review & Commit retains its exact selection after cancellation and a real forced commit failure; only reviewed paths are committed; an unselected modified probe remains uncommitted; and the successful retry performs authoritative targeted reconcile.
