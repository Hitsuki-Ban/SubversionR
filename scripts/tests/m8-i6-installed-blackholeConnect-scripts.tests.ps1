...
$probePath = Join-Path $repoRoot "scripts\release\probe-m8-i6-installed-blackholeConnect.ps1"
...
Assert-True (Test-Path -LiteralPath $probePath -PathType Leaf) "Installed blackholeConnect probe must exist."
$tokens = $null
$parseErrors = $null
$ast = [System.Management.Automation.Language.Parser]::ParseFile($probePath, [ref]$tokens, [ref]$parseErrors)
Assert-True ($parseErrors.Count -eq 0) "Installed blackholeConnect probe must parse without PowerShell errors."
...
Assert-True (($parameterNames -join ",") -ceq ($expectedParameters -join ",")) "Installed blackholeConnect probe must expose only the exact required parameters."

$source = Get-Content -Raw -LiteralPath $probePath
foreach ($required in @(
    'subversionr.diagnostics.installedSvnAnonymousBlackholeConnectReport',
    'SUBVERSIONR_INSTALLED_SVN_ANONYMOUS_BLACKHOLE_CONNECT_REPORT_TOKEN',
    'SUBVERSIONR_INSTALLED_I6_BLACKHOLE_CONNECT_OPERATION_ID',
    ...
    '$repositoryUri.AbsolutePath -ceq "/repo/trunk"',
    'subversionr.release.m8-i6-installed-vsix-blackhole-connect.v1',
    ...
    'Assert-ExactProperties $report.daemonState',
    'Assert-ExactProperties $report.recovery',
    'Assert-ExactProperties $report.blackholeSettlement',
    'nativeLaneReleased',
  ...
  Assert-True ($source.Contains($required)) "Installed blackholeConnect probe is missing the contract lock: $required"
}
foreach ($forbidden in @(
  ...
  Assert-True (-not $source.Contains($forbidden)) "Installed blackholeConnect probe must not contain the forbidden fallback/destructive route: $forbidden"
}
...
$helperSources = foreach ($functionName in $helperNames) {
  ...
  Assert-True ($matches.Count -eq 1) "Installed blackholeConnect probe must define exactly one $functionName helper."
...
try {
  ...
  Assert-True ($contentEntries.Count -eq 2) "Installed blackholeConnect content snapshot must record the complete non-metadata topology."
  $directoryEntry = @($contentEntries | Where-Object { $_.kind -ceq "directory" })
  Assert-True ($directoryEntry.Count -eq 1) "Installed blackholeConnect content snapshot must record the empty directory."
  Assert-ExactProperties $directoryEntry[0] @("kind", "path") "installed blackholeConnect directory snapshot entry"
  Assert-True ([string]$directoryEntry[0].path -ceq "empty-directory") "Installed blackholeConnect directory snapshot path was invalid."
  $fileEntry = @($contentEntries | Where-Object { $_.kind -ceq "file" })
  Assert-True ($fileEntry.Count -eq 1) "Installed blackholeConnect content snapshot must record the sentinel file."
  Assert-ExactProperties $fileEntry[0] @("kind", "path", "sha256", "sizeBytes") "installed blackholeConnect file snapshot entry"
  Assert-True (
  ...
  ) "Installed blackholeConnect file snapshot proof was invalid."
  ...
  Assert-ScriptThrowsContaining {
    Assert-WorkingCopyPreserved $workingCopyRoot $contentBefore $databaseBefore.sizeBytes $databaseBefore.sha256
  } "changed working-copy user content" "Installed blackholeConnect preservation must reject directory deletion."
  New-Item -ItemType Directory -Path $emptyDirectoryPath | Out-Null
  ...
  Assert-ScriptThrowsContaining {
    Assert-WorkingCopyPreserved $workingCopyRoot $contentBefore $databaseBefore.sizeBytes $databaseBefore.sha256
  } "changed working-copy user content" "Installed blackholeConnect preservation must reject directory addition."
  ...
  Assert-ScriptThrowsContaining {
    Assert-WorkingCopyPreserved $workingCopyRoot $contentBefore $databaseBefore.sizeBytes $databaseBefore.sha256
  } "changed working-copy user content" "Installed blackholeConnect preservation must reject user-content mutation."
  [System.IO.File]::WriteAllText($sentinelPath, "controlled sentinel`n", [System.Text.UTF8Encoding]::new($false))

  [System.IO.File]::AppendAllText((Join-Path $metadataRoot "wc.db"), "mutation", [System.Text.UTF8Encoding]::new($false))
  Assert-ScriptThrowsContaining {
    Assert-WorkingCopyPreserved $workingCopyRoot $contentBefore $databaseBefore.sizeBytes $databaseBefore.sha256
  } "database size changed" "Installed blackholeConnect preservation must reject working-copy database mutation."
  [System.IO.File]::WriteAllText((Join-Path $metadataRoot "wc.db"), "controlled-wc-database", [System.Text.UTF8Encoding]::new($false))

  [System.IO.File]::WriteAllText((Join-Path $metadataRoot "wc.db"), "controlled-wc-databasf", [System.Text.UTF8Encoding]::new($false))
  Assert-ScriptThrowsContaining {
    Assert-WorkingCopyPreserved $workingCopyRoot $contentBefore $databaseBefore.sizeBytes $databaseBefore.sha256
  } "database hash changed" "Installed blackholeConnect preservation must reject same-size working-copy database mutation."
  [System.IO.File]::WriteAllText((Join-Path $metadataRoot "wc.db"), "controlled-wc-database", [System.Text.UTF8Encoding]::new($false))
...
if (@($arguments | Where-Object { $_ -like "--extensionTestsPath=*" }).Count -eq 1) {
  ...
  $report = [ordered]@{
    ...
    recovery = [ordered]@{ required = $false; storedKind = "notRequired"; indeterminateObserved = $false }
    blackholeSettlement = [ordered]@{
      operationIdSha256 = "287b5ddbe2663c620313711df628ca6eb69eb1b49ede26b3efad91031046065e"
      effectivePort = 3690; wireSettlementObserved = $true; daemonTerminalStateObserved = $true
    }
    timing = [ordered]@{ clock = "monotonic"; timeoutMs = 5000; elapsedMs = 6500; cleanupSlackMs = 5000 }
    diagnostics = $null
  ...
  [ordered]@{ extensionId = "hitsuki-ban.subversionr"; extensionVersion = "0.2.5"; extensionPath = $packageRoot; report = $report } |
    ConvertTo-Json -Depth 16 -Compress | Set-Content -LiteralPath $env:SUBVERSIONR_INSTALLED_I6_BLACKHOLE_CONNECT_RESULT -Encoding utf8 -NoNewline
  exit 0
}
...
'@ | Set-Content -LiteralPath $fakeCodeScript -Encoding utf8 -NoNewline
  ...
  Assert-NativeCommandFailsContaining {
    & pwsh -NoProfile -NonInteractive -ExecutionPolicy Bypass -File $probePath `
      ...
      -RepositoryUrl "svn://127.0.0.1:3690/repo/trunk" -OperationId "50000000-0000-4000-8000-000000000002" -OperationTimeoutMilliseconds 5000 `
      -ExpectedProductVersion "0.2.5" -DaemonPath $daemonPath -BridgePath $bridgePath -TimeoutSeconds 60
  } "VsixPath must be an absolute path" "Installed blackholeConnect probe must fail before creating its harness for a relative VSIX path."
  Assert-True (-not (Test-Path -LiteralPath $badFixtureRoot)) "Installed blackholeConnect argument failure must not create the harness root."

  foreach ($invalidTimeout in @(4999, 5001)) {
    ...
    Assert-NativeCommandFailsContaining {
      & pwsh -NoProfile -NonInteractive -ExecutionPolicy Bypass -File $probePath `
        ...
        -ExpectedProductVersion "0.2.5" -DaemonPath $daemonPath -BridgePath $bridgePath -TimeoutSeconds 60
    } "OperationTimeoutMilliseconds" "Installed blackholeConnect probe must reject timeout $invalidTimeout at parameter intake."
    Assert-True (-not (Test-Path -LiteralPath $invalidTimeoutFixtureRoot)) "Installed blackholeConnect timeout rejection must not create the harness root."
  }
  ...
  try {
    ...
    $output = & pwsh -NoProfile -NonInteractive -ExecutionPolicy Bypass -File $probePath `
      ...
      -RepositoryUrl "svn://127.0.0.1:3690/repo/trunk" -OperationId "50000000-0000-4000-8000-000000000002" -OperationTimeoutMilliseconds 5000 `
      -ExpectedProductVersion "0.2.5" -DaemonPath $daemonPath -BridgePath $bridgePath -TimeoutSeconds 60
    Assert-True ($LASTEXITCODE -eq 0) "Installed blackholeConnect fake Code CLI probe must pass."
  }
  finally {
  ...
  Assert-True ([string]$wrapper.schema -ceq "subversionr.release.m8-i6-installed-vsix-blackhole-connect.v1") "Installed blackholeConnect wrapper schema was invalid."
  Assert-True ([string]$wrapper.cell -ceq "blackholeConnect" -and [string]$wrapper.status -ceq "passed") "Installed blackholeConnect wrapper identity was invalid."
  Assert-True (
    [string]$wrapper.timing.clock -ceq "monotonic" -and [int]$wrapper.timing.timeoutMs -eq 5000 -and
    [double]$wrapper.timing.elapsedMs -eq 6500 -and [int]$wrapper.timing.cleanupSlackMs -eq 5000
  ) "Installed blackholeConnect wrapper timing proof was invalid."
  Assert-True (
    [string]$wrapper.daemonState.terminalKind -ceq "unreachable" -and
    [string]$wrapper.daemonState.terminalReason -ceq "timeout" -and
    [string]$wrapper.recovery.storedKind -ceq "notRequired" -and
    [int]$wrapper.blackholeSettlement.effectivePort -eq 3690
  ) "Installed blackholeConnect wrapper daemon/recovery/binding proof was invalid."
  Assert-True ($wrapper.workingCopyPreserved -eq $true -and [int]$wrapper.temporaryRootsAfter -eq 0 -and [int]$wrapper.checkoutJournalEntriesAfter -eq 0) "Installed blackholeConnect wrapper cleanup proof was invalid."
  Assert-WorkingCopyPreserved $workingCopyRoot $contentBefore $databaseBefore.sizeBytes $databaseBefore.sha256
}
finally {
...
Write-Host "M8 I6 installed blackholeConnect probe script tests passed."
