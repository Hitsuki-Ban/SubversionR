...
try {
  ...
  Assert-Equal "subversionr.release.packaged-native-version-evidence.v2" $report.nativeCompatibility.schema "VSIX package evidence should persist the packaged native worker-isolation probe."
  ...
  Assert-Equal "33" ([string]$report.nativeCompatibility.protocol.minor) "VSIX package evidence should record the exact credential-settlement protocol minor."
  ...
  Assert-Equal "subversionr.private.credential-provider-probe.v1" $report.nativeCompatibility.credentialProviderProbe.schema "VSIX package evidence should bind the private provider probe schema."
  Assert-Equal "passed" $report.nativeCompatibility.credentialProviderProbe.status "VSIX package evidence should record a passing credential provider probe."
  Assert-Equal "False" ([string]$report.nativeCompatibility.credentialProviderProbe.networkAccess) "VSIX package credential provider evidence must prove no network access."
  Assert-Equal "5" ([string]$report.nativeCompatibility.credentialProviderProbe.scenarios.Count) "VSIX package evidence should record all credential provider scenarios."
  Assert-Equal "firstSave" $report.nativeCompatibility.credentialProviderProbe.scenarios[0].scenario "VSIX package evidence should record the first/save scenario."
  Assert-Equal "request:initial|settle:accepted" ($report.nativeCompatibility.credentialProviderProbe.scenarios[0].events -join "|") "VSIX package evidence should record first/save settlement."
  Assert-Equal "firstNextSave" $report.nativeCompatibility.credentialProviderProbe.scenarios[1].scenario "VSIX package evidence should record the first/next/save scenario."
  Assert-Equal "request:initial|settle:rejected|request:retryAfterRejected|settle:accepted" ($report.nativeCompatibility.credentialProviderProbe.scenarios[1].events -join "|") "VSIX package evidence should record retry after rejected credentials."
  Assert-Equal "request:initial|settle:unused" ($report.nativeCompatibility.credentialProviderProbe.scenarios[2].events -join "|") "VSIX package evidence should record unused settlement."
  Assert-Equal "request:initial|settle:cancelled" ($report.nativeCompatibility.credentialProviderProbe.scenarios[3].events -join "|") "VSIX package evidence should record cancelled settlement."
  Assert-Equal "request:initial|settle:timedOut" ($report.nativeCompatibility.credentialProviderProbe.scenarios[4].events -join "|") "VSIX package evidence should record timed-out settlement."
  Assert-Equal "passed" $report.nativeCompatibility.localDiscovery.status "VSIX package evidence should retain the packaged local discovery smoke."
  Assert-Equal "0" ([string]$report.nativeCompatibility.localDiscovery.candidateCount) "VSIX package evidence should record the empty local discovery fixture."
  Assert-Equal "0" ([string]$report.nativeCompatibility.localDiscovery.fileExternalBoundaryCount) "VSIX package evidence should record no local discovery boundaries."
  Assert-Equal "repository/checkout" $report.nativeCompatibility.workerIsolation.operation "VSIX package evidence should record the synthetic worker operation."
  Assert-Equal "https" $report.nativeCompatibility.workerIsolation.expectedOriginScheme "VSIX package evidence should record the synthetic HTTPS origin scheme without a raw URL."
  Assert-Equal "SUBVERSIONR_REMOTE_TRANSPORT_UNSUPPORTED" $report.nativeCompatibility.workerIsolation.resultCode "VSIX package evidence should prove the isolated foundation stops before transport enablement."
  Assert-Equal "passed" $report.nativeCompatibility.workerIsolation.tempRootCleanup.status "VSIX package evidence should prove worker temp-root cleanup."
  Assert-Equal "0" ([string]$report.nativeCompatibility.workerIsolation.tempRootCleanup.residualEntryCount) "VSIX package evidence should prove no operation temp roots remain."
  Assert-Equal "passed" $report.nativeCompatibility.workerIsolation.sameLaneSubsequent.status "VSIX package evidence should prove the same native lane is reusable after cleanup."
  ...
  Assert-Equal "33" ([string]$report.nativeCompatibility.workerIsolation.subsequentDiagnostics.protocol.minor) "VSIX package evidence should bind subsequent diagnostics to protocol v1.33."
