...
function New-ProvenanceFixture([string]$Root) {
  ...
  Write-JsonFile $vsixEvidencePath ([pscustomobject]@{
      ...
      vsix = [pscustomobject]@{
        path = $vsixPath
        relativePath = "target/tests/release-provenance-scripts/artifacts/subversionr-win32-x64-0.2.5.vsix"
...
"@ | Set-Content -LiteralPath $ghShimPath -Encoding utf8
  ...
  Assert-Equal "subversionr-win32-x64-0.2.5.vsix" $report.candidateAttestation.subjectName "Current candidate attestation should bind the 0.2.5 VSIX name."
