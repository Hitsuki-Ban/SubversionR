...
function New-PublicationGapsFixture([string]$Root) {
  ...
  Write-JsonFile $vsixEvidencePath ([pscustomobject]@{
      ...
      vsix = [pscustomobject]@{
        path = $vsixPath
        relativePath = "target/tests/release-publication-gaps-scripts/artifacts/subversionr-win32-x64-0.2.3.vsix"
  ...
  Write-JsonFile $provenancePath ([pscustomobject]@{
      ...
      candidateAttestation = [pscustomobject]@{
        ...
        subjectName = "subversionr-win32-x64-0.2.3.vsix"
...
try {
  ...
  Assert-Equal "pending-release-attestation" $report.currentCandidate.status "Publication gaps should keep the current 0.2.3 candidate pending before release."
  ...
  Assert-True ($historicalReleasedVsix.sha256 -ne $report.artifacts.vsix.sha256) "Historical 0.2.0 and current 0.2.3 VSIX digests must remain distinct."
  foreach ($blocker in @(
