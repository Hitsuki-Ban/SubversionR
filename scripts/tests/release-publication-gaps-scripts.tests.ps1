...
function New-PublicationGapsFixture([string]$Root) {
  ...
  $vsixPath = Join-Path $artifactsRoot "subversionr-win32-x64-0.2.5.vsix"
  ...
  Write-JsonFile $vsixEvidencePath ([pscustomobject]@{
      ...
      vsix = [pscustomobject]@{
        path = $vsixPath
        relativePath = "target/tests/release-publication-gaps-scripts/artifacts/subversionr-win32-x64-0.2.5.vsix"
  ...
  Write-JsonFile $provenancePath ([pscustomobject]@{
      ...
      candidateAttestation = [pscustomobject]@{
        ...
        subjectName = "subversionr-win32-x64-0.2.5.vsix"
...
try {
  ...
  Assert-Equal "pending-release-attestation" $report.currentCandidate.status "Publication gaps should keep the current 0.2.5 candidate pending before release."
  ...
  Assert-True ($historicalReleasedVsix.sha256 -ne $report.artifacts.vsix.sha256) "Historical 0.2.0 and current 0.2.5 VSIX digests must remain distinct."
  foreach ($blocker in @(
