...
const probe = path.join(repositoryRoot, "scripts", "release", "probe-m8-i6-packaged-stalled-read.mjs");
const schema = "subversionr.release.m8-i6-packaged-native-stalled-read.v1";
const operationId = "12345678-1234-4abc-8def-1234567890ab";

test("executes the exact failure-only stalled remote read and proves the local lane remains available", async () => {
  ...
  try {
    ...
    assert.equal(capture.config.clientName, "subversionr-m8-i6-packaged-stalled-read-evidence");
    ...
    assert.equal(statusRequest.remote.trustEpoch, 7);
    ...
    assert.deepEqual(statusRequest.remote.profile, {
      ...
      profileId: "m8-i6-loopback-anonymous-stalled-read",
    ...
    assert.deepEqual(capture.requests[2], {
      ...
      params: { repositoryId: "repo-id", epoch: 7 },
    });
  } finally {
...
test("rejects every non-exact timeout settlement, safeArgs shape, and diagnostics value", async () => {
  const cases = [
    [{ code: "SUBVERSIONR_REMOTE_RECOVERY_BLOCKED" }, "SUBVERSIONR_I6_PACKAGED_STALLED_READ_ERROR_INVALID"],
    [{ category: "recovery" }, "SUBVERSIONR_I6_PACKAGED_STALLED_READ_ERROR_INVALID"],
    [{ messageKey: "error.remote.recoveryBlocked" }, "SUBVERSIONR_I6_PACKAGED_STALLED_READ_ERROR_INVALID"],
    [{ retryable: true }, "SUBVERSIONR_I6_PACKAGED_STALLED_READ_ERROR_INVALID"],
    [{ safeArgsExtra: true }, "SUBVERSIONR_I6_PACKAGED_STALLED_READ_ERROR_INVALID"],
    [{ failureCategory: "timeout" }, "SUBVERSIONR_I6_PACKAGED_STALLED_READ_FAILURE_INVALID"],
    [{ failureReason: "remoteRecoveryBlocked" }, "SUBVERSIONR_I6_PACKAGED_STALLED_READ_FAILURE_INVALID"],
    [{ cleanupAppropriate: true }, "SUBVERSIONR_I6_PACKAGED_STALLED_READ_FAILURE_INVALID"],
    [{ errorDiagnostics: {} }, "SUBVERSIONR_I6_PACKAGED_STALLED_READ_ERROR_INVALID"],
  ];
...
test("rejects wrong local settlement proof, credential traffic, residue, and redaction leaks", async () => {
  const cases = [
    [{ snapshotRepositoryId: "other" }, "SUBVERSIONR_I6_PACKAGED_STALLED_READ_LOCAL_SNAPSHOT_INVALID"],
    [{ snapshotEpoch: 8 }, "SUBVERSIONR_I6_PACKAGED_STALLED_READ_LOCAL_SNAPSHOT_INVALID"],
    [{ snapshotSource: "remote-worker" }, "SUBVERSIONR_I6_PACKAGED_STALLED_READ_LOCAL_SNAPSHOT_INVALID"],
    [{ credentialRequest: true }, "SUBVERSIONR_I6_PACKAGED_STALLED_READ_CREDENTIAL_HANDLER_INVOKED"],
    [{ residue: true }, "SUBVERSIONR_I6_PACKAGED_STALLED_READ_WORKER_TEMP_RESIDUE"],
    [{ diagnosticsLeak: "repository" }, "SUBVERSIONR_I6_PACKAGED_STALLED_READ_DIAGNOSTICS_LEAK"],
    [{ diagnosticsLeak: "operation" }, "SUBVERSIONR_I6_PACKAGED_STALLED_READ_DIAGNOSTICS_LEAK"],
    [{ tamperUserContent: true }, "SUBVERSIONR_I6_PACKAGED_STALLED_READ_WORKING_COPY_INVALID"],
    [{ tamperWcDatabaseSameSize: true }, "SUBVERSIONR_I6_PACKAGED_STALLED_READ_WORKING_COPY_INVALID"],
    [{ protocolMinor: 34 }, "SUBVERSIONR_I6_PACKAGED_STALLED_READ_CAPABILITY_INVALID"],
    [{ remoteSvnAnonymous: false }, "SUBVERSIONR_I6_PACKAGED_STALLED_READ_CAPABILITY_INVALID"],
    [{ trustDropsAfterTimeout: true }, "SUBVERSIONR_I6_PACKAGED_STALLED_READ_TRUST_INVALID"],
    [{ closeFalse: true }, "SUBVERSIONR_I6_PACKAGED_STALLED_READ_CLOSE_INVALID"],
  ];
  for (const [behavior, expectedCode] of cases) {
    const fixture = await createFixture(behavior);
    try {
...
test("rejects non-exact CLI keys, non-canonical external IDs, non-loopback origins, and non-empty profiles", async () => {
  const fixture = await createFixture({});
  try {
    const argumentCases = [
      [...fixture.args, "--extra", "value"],
      replaceValue(fixture.args, "--timeout-ms", "030000"),
      replaceValue(fixture.args, "--operation-id", "12345678-1234-4ABC-8def-1234567890ab"),
    ...
    assert.deepEqual(JSON.parse(nonEmpty.stdout), failed("SUBVERSIONR_I6_PACKAGED_STALLED_READ_FILESYSTEM_INVALID"));
  } finally {
    await rm(fixture.root, { recursive: true, force: true });
  }
  ...
  try {
    ...
    assert.equal(result.status, 1);
    assert.deepEqual(JSON.parse(result.stdout), failed("SUBVERSIONR_I6_PACKAGED_STALLED_READ_OPEN_INVALID"));
  } finally {
  ...
  try {
    ...
    assert.equal(result.status, 1);
    assert.deepEqual(JSON.parse(result.stdout), failed("SUBVERSIONR_I6_PACKAGED_STALLED_READ_ORIGIN_INVALID"));
  } finally {
...
async function createFixture(behavior) {
  const root = await mkdtemp(path.join(os.tmpdir(), "subversionr-i6-packaged-stalled-read-test-"));
  ...
  await mkdir(path.join(workingCopyPath, ".svn"));
  ...
  await writeFile(path.join(workingCopyPath, "tracked.txt"), "controlled user content\n", "utf8");
  ...
  await writeFile(backendModule, fakeBackendSource(behavior, workingCopyPath, repositoryUrl, operationId), "utf8");
...
function fakeBackendSource(behavior, workingCopyPath, repositoryUrl, externalOperationId) {
  return `
...
exports.startBackendProcess = async function startBackendProcess(config, handlers) {
  ...
  return {
    ...
    currentRemoteTrustEpoch() {
      return behavior.trustDropsAfterTimeout === true && capture.methods.includes("status/checkRemote") ? 8 : 7;
    },
    async sendRequest(method, params) {
      ...
      if (method === "status/checkRemote") {
        ...
        if (behavior.tamperUserContent === true) {
          fs.writeFileSync(path.join(workingCopyPath, "tracked.txt"), "tampered user content\\n");
        }
        if (behavior.tamperWcDatabaseSameSize === true) {
          const wcDatabasePath = path.join(workingCopyPath, ".svn", "wc.db");
          const bytes = fs.readFileSync(wcDatabasePath);
          bytes[0] ^= 0xff;
          fs.writeFileSync(wcDatabasePath, bytes);
        }
        throw error;
      }
      if (method === "status/getSnapshot") return {
        repositoryId: behavior.snapshotRepositoryId ?? "repo-id",
        epoch: behavior.snapshotEpoch ?? 7,
      ...
      if (method === "repository/close") return {
        repositoryId: "repo-id", epoch: 7, closed: behavior.closeFalse !== true,
      };
...
exports.canonicalEndpointFromRepositoryUrl = function(repositoryUrl) {
  ...
  return {
    scheme: "svn",
    canonicalHost: behavior.canonicalHost ?? url.hostname,
