...
try {
  ...
  Assert-True ($workflowContent -match "async function collectFreshnessReportUntilPathsAbsent") "Installed Source Control UI E2E harness should support condition-based freshness waits for committed paths to leave the SourceControl projection."
  ...
  Assert-True ($workflowContent -match '(?s)async function runCommitSelectedWorkflow.*?expectedRefreshTarget\s*=\s*\{\s*path:\s*selected\.path,\s*depth:\s*"empty",\s*reason:\s*"operationCommit"\s*\}.*?postCommitObservation\s*=\s*await collectFreshnessReportUntilPathsAbsent\(.*?\[selected\.path\],\s*commitSelectedOpenReport,\s*\[expectedRefreshTarget\],\s*30000\s*\).*?postCommitFreshnessReport\s*=\s*postCommitObservation\.report.*?unselectedResourceAfter\s*=\s*findResource\(postCommitFreshnessReport.*?coverageReport\s*=\s*postCommitObservation\.coverageReport') "Commit Selected should wait for the committed path to leave the final SourceControl projection and for exact targeted refresh coverage while checking the unselected resource in that same report."
  Assert-True ($workflowContent -match '\$commitSelectedSummary\s*=\s*\$Result\.commitSelectedReport\s*\|\s*ConvertTo-Json -Depth 8 -Compress') "Commit Selected result validation should include the actual report in failure diagnostics."
