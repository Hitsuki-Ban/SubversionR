...
$probePath = Join-Path $repoRoot "scripts\release\probe-m8-i6-installed-cancellation.ps1"
...
Assert-True (Test-Path -LiteralPath $probePath -PathType Leaf) "Installed cancellation probe must exist."
$tokens = $null
$parseErrors = $null
$ast = [System.Management.Automation.Language.Parser]::ParseFile($probePath, [ref]$tokens, [ref]$parseErrors)
Assert-True ($parseErrors.Count -eq 0) "Installed cancellation probe must parse without PowerShell errors."
...
Assert-True (($parameterNames -join ",") -ceq ($expectedParameters -join ",")) "Installed cancellation probe must expose only the exact required parameters."
...
foreach ($required in @(
    ...
    'SUBVERSIONR_INSTALLED_I6_CANCELLATION_OPERATION_ID',
    'SUBVERSIONR_INSTALLED_I6_CANCELLATION_FIXTURE_STATE_PATH',
    'externally supplied canonical UUID',
    'token, repositoryUrl, workingCopyPath, operationId, fixtureStatePath',
    'subversionr.release.m8-i6-installed-vsix-cancellation.v1',
    'Assert-ExactProperties $report.cancellationSettlement @("trigger", "localCode", "wireCode", "wireReason", "wireSettlementObserved")',
    'cancellationSettlement = $report.cancellationSettlement',
    'SUBVERSIONR_REMOTE_WORKER_CANCELLED',
    'operationCancelled',
    'nativeLaneReleased',
    'localSnapshotAfterCancellation',
  ...
  Assert-True ($source.Contains($required)) "Installed cancellation probe is missing the contract lock: $required"
}
foreach ($forbidden in @(
    ...
    'SUBVERSIONR_INSTALLED_I6_CANCELLATION_TIMEOUT_MS',
    'Get-WmiObject',
    'Register-WmiEvent',
    'svn.exe',
    'svnadmin.exe',
  ...
  Assert-True (-not $source.Contains($forbidden)) "Installed cancellation probe must not contain the forbidden fallback/destructive route: $forbidden"
}
...
$helperSources = foreach ($functionName in $helperNames) {
  ...
  Assert-True ($matches.Count -eq 1) "Installed cancellation probe must define exactly one $functionName helper."
...
try {
  ...
  Assert-True ($contentEntries.Count -eq 2) "Installed cancellation content snapshot must record the complete non-metadata topology."
  $directoryEntry = @($contentEntries | Where-Object { $_.kind -ceq "directory" })
  Assert-True ($directoryEntry.Count -eq 1) "Installed cancellation content snapshot must record the empty directory."
  Assert-ExactProperties $directoryEntry[0] @("kind", "path") "installed cancellation directory snapshot entry"
  Assert-True ([string]$directoryEntry[0].path -ceq "empty-directory") "Installed cancellation directory snapshot path was invalid."
  $fileEntry = @($contentEntries | Where-Object { $_.kind -ceq "file" })
  Assert-True ($fileEntry.Count -eq 1) "Installed cancellation content snapshot must record the sentinel file."
  Assert-ExactProperties $fileEntry[0] @("kind", "path", "sha256", "sizeBytes") "installed cancellation file snapshot entry"
  Assert-True (
  ...
  ) "Installed cancellation file snapshot proof was invalid."
  ...
  Assert-ScriptThrowsContaining {
    Assert-WorkingCopyPreserved $workingCopyRoot $contentBefore $databaseBefore.sizeBytes $databaseBefore.sha256
  } "changed working-copy user content" "Installed cancellation preservation must reject directory deletion."
  New-Item -ItemType Directory -Path $emptyDirectoryPath | Out-Null
  ...
  Assert-ScriptThrowsContaining {
    Assert-WorkingCopyPreserved $workingCopyRoot $contentBefore $databaseBefore.sizeBytes $databaseBefore.sha256
  } "changed working-copy user content" "Installed cancellation preservation must reject directory addition."
  ...
  Assert-ScriptThrowsContaining {
    Assert-WorkingCopyPreserved $workingCopyRoot $contentBefore $databaseBefore.sizeBytes $databaseBefore.sha256
  } "changed working-copy user content" "Installed cancellation preservation must reject user-content mutation."
  [System.IO.File]::WriteAllText($sentinelPath, "controlled sentinel`n", [System.Text.UTF8Encoding]::new($false))

  [System.IO.File]::AppendAllText((Join-Path $metadataRoot "wc.db"), "mutation", [System.Text.UTF8Encoding]::new($false))
  Assert-ScriptThrowsContaining {
    Assert-WorkingCopyPreserved $workingCopyRoot $contentBefore $databaseBefore.sizeBytes $databaseBefore.sha256
  } "database size changed" "Installed cancellation preservation must reject working-copy database mutation."
  [System.IO.File]::WriteAllText((Join-Path $metadataRoot "wc.db"), "controlled-wc-database", [System.Text.UTF8Encoding]::new($false))

  [System.IO.File]::WriteAllText((Join-Path $metadataRoot "wc.db"), "controlled-wc-databasf", [System.Text.UTF8Encoding]::new($false))
  Assert-ScriptThrowsContaining {
    Assert-WorkingCopyPreserved $workingCopyRoot $contentBefore $databaseBefore.sizeBytes $databaseBefore.sha256
  } "database hash changed" "Installed cancellation preservation must reject same-size working-copy database mutation."
  [System.IO.File]::WriteAllText((Join-Path $metadataRoot "wc.db"), "controlled-wc-database", [System.Text.UTF8Encoding]::new($false))
...
if (@($arguments | Where-Object { $_ -like "--extensionTestsPath=*" }).Count -eq 1) {
  ...
  $fixtureState = Get-Content -Raw -LiteralPath $env:SUBVERSIONR_INSTALLED_I6_CANCELLATION_FIXTURE_STATE_PATH | ConvertFrom-Json
  $fixtureState.connections = 1
  $fixtureState.greetingSent = 1
  $fixtureState.clientResponseReceived = 1
  $fixtureState | ConvertTo-Json -Depth 8 -Compress | Set-Content -LiteralPath $env:SUBVERSIONR_INSTALLED_I6_CANCELLATION_FIXTURE_STATE_PATH -Encoding utf8 -NoNewline
  $report = [ordered]@{
    ...
    settlement = [ordered]@{
      code = "SUBVERSIONR_REMOTE_WORKER_CANCELLED"; category = "cancelled"; messageKey = "error.remote.workerCancelled"; retryable = $false
      remoteFailure = [ordered]@{ category = "cancellation"; reason = "operationCancelled"; cleanupAppropriate = $false }
    }
    cancellationSettlement = [ordered]@{
      trigger = "abort-signal-after-greeting"; localCode = "JSON_RPC_REQUEST_CANCELLED"
      wireCode = "SUBVERSIONR_REMOTE_WORKER_CANCELLED"; wireReason = "operationCancelled"; wireSettlementObserved = $true
    }
    diagnostics = $null
  ...
  [ordered]@{ extensionId = "hitsuki-ban.subversionr"; extensionVersion = "0.2.5"; extensionPath = $packageRoot; report = $report } |
    ConvertTo-Json -Depth 16 -Compress | Set-Content -LiteralPath $env:SUBVERSIONR_INSTALLED_I6_CANCELLATION_RESULT -Encoding utf8 -NoNewline
  exit 0
}
...
'@ | Set-Content -LiteralPath $fakeCodeScript -Encoding utf8 -NoNewline
  ...
  } "VsixPath must be an absolute path" "Installed cancellation probe must fail before creating its harness for a relative VSIX path."
  Assert-True (-not (Test-Path -LiteralPath $badFixtureRoot)) "Installed cancellation argument failure must not create the harness root."
  ...
  Assert-NativeCommandFailsContaining {
    & pwsh -NoProfile -NonInteractive -ExecutionPolicy Bypass -File $probePath `
      ...
      -ExpectedProductVersion "0.2.5" -DaemonPath $daemonPath -BridgePath $bridgePath -TimeoutSeconds 60
  } "FixtureStatePath must be an absolute path" "Installed cancellation probe must reject a relative fixture state path."
  Assert-True (-not (Test-Path -LiteralPath $invalidStateFixtureRoot)) "Installed cancellation fixture-state rejection must not create the harness root."
  ...
  try {
    ...
    $output = & pwsh -NoProfile -NonInteractive -ExecutionPolicy Bypass -File $probePath `
      ...
      -RepositoryUrl "svn://127.0.0.1:3690/repo/trunk" -FixtureStatePath $fixtureStatePath -OperationId "50000000-0000-4000-8000-000000000002" `
      -ExpectedProductVersion "0.2.5" -DaemonPath $daemonPath -BridgePath $bridgePath -TimeoutSeconds 60
    Assert-True ($LASTEXITCODE -eq 0) "Installed cancellation fake Code CLI probe must pass."
  }
  finally {
  ...
  Assert-True ([string]$wrapper.schema -ceq "subversionr.release.m8-i6-installed-vsix-cancellation.v1") "Installed cancellation wrapper schema was invalid."
  Assert-True ([string]$wrapper.cell -ceq "cancellation" -and [string]$wrapper.status -ceq "passed") "Installed cancellation wrapper identity was invalid."
  Assert-True (
    [string]$wrapper.cancellationSettlement.trigger -ceq "abort-signal-after-greeting" -and
    [string]$wrapper.cancellationSettlement.localCode -ceq "JSON_RPC_REQUEST_CANCELLED" -and
    [string]$wrapper.cancellationSettlement.wireCode -ceq "SUBVERSIONR_REMOTE_WORKER_CANCELLED" -and
    [string]$wrapper.cancellationSettlement.wireReason -ceq "operationCancelled" -and
    $wrapper.cancellationSettlement.wireSettlementObserved -eq $true
  ) "Installed cancellation wrapper dual settlement proof was invalid."
  Assert-True ($wrapper.workingCopyPreserved -eq $true -and [int]$wrapper.temporaryRootsAfter -eq 0 -and [int]$wrapper.checkoutJournalEntriesAfter -eq 0) "Installed cancellation wrapper cleanup proof was invalid."
  Assert-WorkingCopyPreserved $workingCopyRoot $contentBefore $databaseBefore.sizeBytes $databaseBefore.sha256
}
finally {
...
Write-Host "M8 I6 installed cancellation probe script tests passed."
