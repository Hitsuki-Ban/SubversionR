...
<PackageManifest Version="2.0.0" xmlns="http://schemas.microsoft.com/developer/vsx-schema/2011">
  <Metadata>
    <Identity Language="en-US" Id="subversionr" Version="0.2.2" Publisher="hitsuki-ban" TargetPlatform="win32-x64" />
...
"@ | Set-Content -LiteralPath (Join-Path $stagingRoot "extension.vsixmanifest") -NoNewline -Encoding utf8
  @{ name = "subversionr"; publisher = "hitsuki-ban"; version = "0.2.2" } |
...
function New-Fixture([string]$Root, [bool]$PreRelease) {
  ...
  $vsixPath = Join-Path $Root "subversionr-win32-x64-0.2.2.vsix"
...
try {
  ...
  Assert-Equal "v0.2.2-beta.1" $evidence.release.tag "Publication evidence should bind the candidate release."
  Assert-Equal "0.2.2" $evidence.extension.version "Publication evidence should bind the extension version."
  ...
  $tamperedVsixPath = Join-Path $fixture.root "tampered\subversionr-win32-x64-0.2.2.vsix"
