...
'@ | Set-Content -LiteralPath (Join-Path $httpdStage "include\ap_release.h") -Encoding ascii -NoNewline
  ...
  Assert-Equal "1784241450`n" ((Get-Content -Raw -LiteralPath (Join-Path $repoRoot "native\release-build-epoch.txt")).Replace("`r`n", "`n")) "The 0.2.5 native release epoch should remain bound to the final pre-candidate main commit."
