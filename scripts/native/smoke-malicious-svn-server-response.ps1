...
foreach ($requiredPath in @(
  ...
  (Join-Path $bridgeOutputDirectoryResolved.Path "svnserve.exe"),
  ...
  if (-not (Test-Path -LiteralPath $requiredPath -PathType Leaf)) {
    throw "Required malicious SVN server-response smoke artifact is missing: $requiredPath"
  }
}

$env:SUBVERSIONR_TEST_BRIDGE_DLL = $bridgeDll.Path

cargo test -p subversionr-daemon --test native_bridge native_bridge_malicious_svn_server_response_history_log_fails_without_auth_prompts_or_crash -- --ignored --exact
if ($LASTEXITCODE -ne 0) {
  throw "Malicious SVN server-response native bridge smoke test failed with exit code $LASTEXITCODE."
}
