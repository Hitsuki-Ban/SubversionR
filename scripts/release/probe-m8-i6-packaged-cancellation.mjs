...
try {
  ...
  if (
    ...
    throw new Error("SUBVERSIONR_I6_PACKAGED_CANCELLATION_MODULE_INVALID");
  }
  if (!sameEndpoint(canonicalEndpointFromRepositoryUrl(options.repositoryUrl), endpoint)) {
    throw new Error("SUBVERSIONR_I6_PACKAGED_CANCELLATION_ORIGIN_INVALID");
  }
  ...
  connection = await startBackendProcess(
    {
      ...
      remoteStateRoot,
      clientName: "subversionr-m8-i6-packaged-cancellation-evidence",
      clientVersion: "1",
      locale: "en",
    ...
    },
    {
      requestHandler: async (method) => {
        ...
        throw new Error("SUBVERSIONR_I6_PACKAGED_CANCELLATION_CREDENTIAL_HANDLER_FORBIDDEN");
      },
  ...
  await waitForGreetingStall(options.fixtureStatePath, endpoint.effectivePort);
  ...
  let wireError;
  try {
    await connection.waitForCancelledRequestWireSettlement(requestId, WIRE_SETTLEMENT_TIMEOUT_MS);
  } catch (error) {
    wireError = error;
  }
  ...
  if (temporaryRoots.length !== 0) throw new Error("SUBVERSIONR_I6_PACKAGED_CANCELLATION_WORKER_TEMP_RESIDUE");
  ...
  if (temporaryRootsAfterShutdown.length !== 0) {
    throw new Error("SUBVERSIONR_I6_PACKAGED_CANCELLATION_WORKER_TEMP_RESIDUE");
  }
...
async function captureImmediateRejection(request) {
  ...
  if (observed.state !== "rejected") {
    throw new Error("SUBVERSIONR_I6_PACKAGED_CANCELLATION_LOCAL_SETTLEMENT_INVALID");
  }
  return observed.error;
}

function requireLocalCancellation(error) {
  if (
    !isRecord(error) ||
    error.name !== "JsonRpcRequestCancelledError" ||
...
function requireWireCancellation(error) {
  if (
    !isRecord(error) ||
    ...
    throw new Error("SUBVERSIONR_I6_PACKAGED_CANCELLATION_WIRE_SETTLEMENT_INVALID");
  }
  ...
  if (
    ...
    failure.reason !== "operationCancelled" ||
    failure.cleanupAppropriate !== false
  ) {
    throw new Error("SUBVERSIONR_I6_PACKAGED_CANCELLATION_FAILURE_INVALID");
  }
}

async function waitForGreetingStall(fixtureStatePath, expectedPort) {
  const deadline = performance.now() + GREETING_OBSERVATION_TIMEOUT_MS;
  while (performance.now() <= deadline) {
    const state = await readFixtureState(fixtureStatePath, expectedPort);
    if (
      state.connections === 1 &&
    ...
    if (
      ...
      throw new Error("SUBVERSIONR_I6_PACKAGED_CANCELLATION_FIXTURE_STATE_INVALID");
    }
    await delay(20);
  }
  throw new Error("SUBVERSIONR_I6_PACKAGED_CANCELLATION_GREETING_NOT_OBSERVED");
}

async function readFixtureState(fixtureStatePath, expectedPort) {
  let value;
  try {
...
function requireLocalSnapshot(value, session) {
  if (!isRecord(value) || value.repositoryId !== session.repositoryId || value.epoch !== session.epoch || value.source !== "libsvn-local") {
    throw new Error("SUBVERSIONR_I6_PACKAGED_CANCELLATION_LOCAL_SNAPSHOT_INVALID");
  }
}
...
function requireDiagnosticsRedacted(value, options) {
  if (
    ...
    throw new Error("SUBVERSIONR_I6_PACKAGED_CANCELLATION_DIAGNOSTICS_INVALID");
  }
...
function requireRedacted(value, options) {
  ...
  for (const sensitive of [
    ...
    if (lowered.includes(escaped)) throw new Error("SUBVERSIONR_I6_PACKAGED_CANCELLATION_DIAGNOSTICS_LEAK");
  }
}
...
function requireCapabilities(activeConnection) {
  ...
  if (
    ...
    typeof activeConnection.waitForCancelledRequestWireSettlement !== "function"
  ) {
    throw new Error("SUBVERSIONR_I6_PACKAGED_CANCELLATION_CAPABILITY_INVALID");
  }
...
function requireStableTrustAndNoCredentials(activeConnection, trustEpoch, inboundMethods) {
  if (!activeConnection.isRemoteSubmissionEnabled() || activeConnection.currentRemoteTrustEpoch() !== trustEpoch) {
    throw new Error("SUBVERSIONR_I6_PACKAGED_CANCELLATION_TRUST_INVALID");
  }
  if (inboundMethods.length !== 0) throw new Error("SUBVERSIONR_I6_PACKAGED_CANCELLATION_CREDENTIAL_HANDLER_INVOKED");
}
...
function requireOpenResponse(value, workingCopyPath, endpoint) {
  if (
    ...
    throw new Error("SUBVERSIONR_I6_PACKAGED_CANCELLATION_OPEN_INVALID");
  }
  let repositoryRoot;
  try {
...
function requireEnvelope(envelope, options, endpoint, trustEpoch) {
  if (
    ...
    envelope.profile.profileId !== "m8-i6-loopback-anonymous-cancellation" ||
    ...
    throw new Error("SUBVERSIONR_I6_PACKAGED_CANCELLATION_ENVELOPE_INVALID");
  }
}
...
function requireCloseResponse(value, session) {
  if (
    ...
    throw new Error("SUBVERSIONR_I6_PACKAGED_CANCELLATION_CLOSE_INVALID");
  }
}
...
function parseRepositoryUrl(value) {
  ...
  try {
    ...
    throw new Error("SUBVERSIONR_I6_PACKAGED_CANCELLATION_URL_INVALID");
  }
  if (
...
function parseOptions(args) {
  ...
  if (args.length !== names.length * 2) throw new Error("SUBVERSIONR_I6_PACKAGED_CANCELLATION_ARGUMENT_INVALID");
  const parsed = {};
  for (let index = 0; index < args.length; index += 2) {
    const rawName = args[index];
    const rawValue = args[index + 1];
    if (typeof rawName !== "string" || !rawName.startsWith("--") || typeof rawValue !== "string") {
      throw new Error("SUBVERSIONR_I6_PACKAGED_CANCELLATION_ARGUMENT_INVALID");
    }
    const name = rawName.slice(2);
    if (!names.includes(name) || name in parsed) throw new Error("SUBVERSIONR_I6_PACKAGED_CANCELLATION_ARGUMENT_INVALID");
...
async function requireFilesystemContract(options, endpoint) {
  ...
  if (!profile.isDirectory() || (await readdir(options.profileRoot)).length !== 0 || !workingCopy.isDirectory()) {
    throw new Error("SUBVERSIONR_I6_PACKAGED_CANCELLATION_FILESYSTEM_INVALID");
  }
...
async function requireWorkingCopyPreserved(workingCopyPath, expected) {
  ...
  if (
    ...
    throw new Error("SUBVERSIONR_I6_PACKAGED_CANCELLATION_WORKING_COPY_INVALID");
  }
}
...
async function captureUserContent(workingCopyPath, relativeDirectory, entries) {
  ...
  for (const child of children) {
    ...
    if (!child.isFile()) throw new Error("SUBVERSIONR_I6_PACKAGED_CANCELLATION_WORKING_COPY_INVALID");
...
async function requireFile(file) {
  ...
  throw new Error("SUBVERSIONR_I6_PACKAGED_CANCELLATION_FILE_INVALID");
}
...
function requireAbsolute(value) {
  if (!path.isAbsolute(value)) throw new Error("SUBVERSIONR_I6_PACKAGED_CANCELLATION_ARGUMENT_INVALID");
...
function safeErrorCode(error) {
  return isRecord(error) && typeof error.message === "string" && /^[A-Z0-9_]+$/u.test(error.message)
    ? error.message
    : "SUBVERSIONR_I6_PACKAGED_CANCELLATION_FAILED";
}
