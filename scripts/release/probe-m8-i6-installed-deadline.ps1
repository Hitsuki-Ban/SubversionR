...
$DeadlineCommand = "subversionr.diagnostics.installedSvnAnonymousDeadlineReport"
$DeadlineTokenEnvironment = "SUBVERSIONR_INSTALLED_SVN_ANONYMOUS_DEADLINE_REPORT_TOKEN"
...
function Get-WorkingCopyDatabaseProof([string]$WorkingCopyRoot) {
  ...
  Assert-True (Test-Path -LiteralPath $path -PathType Leaf) "The installed deadline working-copy database is missing."
  $size = [int64](Get-Item -LiteralPath $path).Length
  Assert-True ($size -gt 0) "The installed deadline working-copy database is empty."
...
function Assert-WorkingCopyPreserved(
  ...
  Assert-True (Test-Path -LiteralPath $WorkingCopyRoot -PathType Container) "The installed deadline working copy root was removed."
  Assert-True (Test-Path -LiteralPath (Join-Path $WorkingCopyRoot ".svn") -PathType Container) "The installed deadline working-copy metadata root was removed."
  $actualDatabase = Get-WorkingCopyDatabaseProof $WorkingCopyRoot
  Assert-True ($actualDatabase.sizeBytes -eq $ExpectedDatabaseSize) "The installed deadline working-copy database size changed."
  Assert-True ($actualDatabase.sha256 -ceq $ExpectedDatabaseSha256) "The installed deadline working-copy database hash changed."
  Assert-True ((Get-WorkingCopyContentSnapshot $WorkingCopyRoot) -ceq $ExpectedContentSnapshot) "The installed deadline read-only operation changed working-copy user content."
}
...
function Get-CandidateProcessCount([string]$ExecutablePath) {
  ...
  catch { throw "Installed deadline candidate process cleanup observation through Win32_Process failed." }
}
...
function Wait-CandidateProcessAbsent([string]$ExecutablePath, [int]$DeadlineMilliseconds) {
  ...
  throw "The installed deadline candidate daemon or worker remained alive after the Extension Host exited."
}

function Get-TemporaryRootCount([string]$RemoteWorkersRoot) {
  Assert-True (Test-Path -LiteralPath $RemoteWorkersRoot -PathType Container) "The installed deadline remote-workers root was not created."
...
function Assert-EmptyCheckoutJournal([string]$RemoteStateRoot) {
  ...
  Assert-True (Test-Path -LiteralPath $RemoteStateRoot -PathType Container) "The installed deadline remote-state path was not a directory."
  Assert-True (-not (Test-Path -LiteralPath (Join-Path $RemoteStateRoot $JournalTemporaryFileName))) "The installed deadline execution created a checkout journal temporary file."
  $journalPath = Join-Path $RemoteStateRoot $JournalFileName
  if (-not (Test-Path -LiteralPath $journalPath)) { return }
  $journal = Get-Content -Raw -LiteralPath $journalPath | ConvertFrom-Json
  Assert-ExactProperties $journal @("schemaVersion", "entries") "installed deadline checkout journal"
  Assert-True ([int]$journal.schemaVersion -eq 1 -and @($journal.entries).Count -eq 0) "The installed deadline execution left checkout journal entries."
}
...
@'
{"name":"subversionr-m8-i6-installed-deadline-harness","displayName":"SubversionR M8 I6 Installed Deadline Harness","version":"0.0.0","publisher":"hitsuki-ban-test","private":true,"engines":{"vscode":"^1.101.0"},"main":"./d/extension.js","activationEvents":[]}
...
const COMMAND = "subversionr.diagnostics.installedSvnAnonymousDeadlineReport";
const TOKEN_ENVIRONMENT = "SUBVERSIONR_INSTALLED_SVN_ANONYMOUS_DEADLINE_REPORT_TOKEN";

function requiredEnvironment(name) {
  const value = process.env[name];
  if (typeof value !== "string" || value.length === 0) throw new Error(`Missing required installed deadline environment: ${name}`);
  return value;
}
...
async function run() {
  const resultPath = requiredEnvironment("SUBVERSIONR_INSTALLED_I6_DEADLINE_RESULT");
  const extensionsRoot = requiredEnvironment("SUBVERSIONR_INSTALLED_I6_DEADLINE_EXTENSIONS_ROOT");
  const token = requiredEnvironment(TOKEN_ENVIRONMENT);
  const repositoryUrl = requiredEnvironment("SUBVERSIONR_INSTALLED_I6_DEADLINE_REPOSITORY_URL");
  const workingCopyPath = requiredEnvironment("SUBVERSIONR_INSTALLED_I6_DEADLINE_WORKING_COPY_PATH");
  const operationId = requiredEnvironment("SUBVERSIONR_INSTALLED_I6_DEADLINE_OPERATION_ID");
  if (!/^(?!00000000-0000-0000-0000-000000000000$)[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/.test(operationId)) {
    throw new Error("Installed deadline operation ID must be an externally supplied canonical UUID.");
  }
  const operationTimeoutMs = Number(requiredEnvironment("SUBVERSIONR_INSTALLED_I6_DEADLINE_TIMEOUT_MS"));
  if (operationTimeoutMs !== 500) {
    throw new Error("Installed deadline operation timeout is invalid.");
  }
  ...
  if (extension.isActive) throw new Error("Installed SubversionR extension activated before the deadline command.");
  ...
  }), "installed SVN anonymous deadline command", operationTimeoutMs + 30000);
  if (process.env[TOKEN_ENVIRONMENT] !== undefined) throw new Error("Installed deadline token was not consumed during extension activation.");
  const active = vscode.extensions.getExtension("hitsuki-ban.subversionr");
  if (!active?.isActive || active.extensionPath !== extension.extensionPath) throw new Error("Installed SubversionR extension identity changed during the deadline command.");
  const serialized = JSON.stringify(report).toLowerCase();
  for (const sensitive of [token, repositoryUrl, workingCopyPath, workingCopyPath.replaceAll("\\", "/"), operationId]) {
    if (serialized.includes(sensitive.toLowerCase())) throw new Error("Installed deadline report leaked request identity.");
  }
...
Invoke-Code $codeResolved @("--user-data-dir", $userDataRoot, "--extensions-dir", $extensionsRoot, "--install-extension", $vsixResolved, "--force") 180 "VS Code CLI installed deadline extension install"
$installed = @(& $codeResolved --user-data-dir $userDataRoot --extensions-dir $extensionsRoot --list-extensions --show-versions)
Assert-True ($LASTEXITCODE -eq 0) "VS Code CLI installed deadline extension listing failed."
Assert-True ($installed -contains "hitsuki-ban.subversionr@$ExpectedProductVersion") "Installed deadline extension version did not match ExpectedProductVersion."
...
Assert-True ((Get-Sha256 $installedDaemonPath) -ceq (Get-Sha256 $daemonResolved)) "Installed deadline daemon bytes did not match the candidate daemon."
Assert-True ((Get-Sha256 $installedBridgePath) -ceq (Get-Sha256 $bridgeResolved)) "Installed deadline bridge bytes did not match the candidate bridge."
Assert-True ((Get-CandidateProcessCount $installedDaemonPath) -eq 0) "The installed deadline candidate daemon or worker was already running before the Extension Host probe."

$names = @(
  "SUBVERSIONR_INSTALLED_I6_DEADLINE_RESULT", "SUBVERSIONR_INSTALLED_I6_DEADLINE_EXTENSIONS_ROOT", $DeadlineTokenEnvironment,
  "SUBVERSIONR_INSTALLED_I6_DEADLINE_REPOSITORY_URL", "SUBVERSIONR_INSTALLED_I6_DEADLINE_WORKING_COPY_PATH",
  "SUBVERSIONR_INSTALLED_I6_DEADLINE_OPERATION_ID", "SUBVERSIONR_INSTALLED_I6_DEADLINE_TIMEOUT_MS", "APPDATA", "LOCALAPPDATA", "USERPROFILE", "HOME", "TEMP", "TMP"
)
$previous = @{}
foreach ($name in $names) { $previous[$name] = Get-ProcessEnvironmentValue $name }
try {
  $env:SUBVERSIONR_INSTALLED_I6_DEADLINE_RESULT = $resultPath
  $env:SUBVERSIONR_INSTALLED_I6_DEADLINE_EXTENSIONS_ROOT = $extensionsRoot
  Set-Item -LiteralPath "Env:$DeadlineTokenEnvironment" -Value ([Guid]::NewGuid().ToString("N"))
  $env:SUBVERSIONR_INSTALLED_I6_DEADLINE_REPOSITORY_URL = $RepositoryUrl
  $env:SUBVERSIONR_INSTALLED_I6_DEADLINE_WORKING_COPY_PATH = $workingCopyResolved
  $env:SUBVERSIONR_INSTALLED_I6_DEADLINE_OPERATION_ID = $OperationId
  $env:SUBVERSIONR_INSTALLED_I6_DEADLINE_TIMEOUT_MS = $OperationTimeoutMilliseconds.ToString([Globalization.CultureInfo]::InvariantCulture)
  ...
  ) $TimeoutSeconds "VS Code installed I6 deadline Extension Host probe"
}
...
Assert-True (Test-Path -LiteralPath $resultPath -PathType Leaf) "Installed deadline harness did not write its bounded result."
$result = Get-Content -Raw -LiteralPath $resultPath | ConvertFrom-Json -Depth 64
Assert-ExactProperties $result @("extensionId", "extensionVersion", "extensionPath", "report") "installed deadline harness result"
Assert-True ([string]$result.extensionId -ceq "hitsuki-ban.subversionr") "Installed deadline extension identity was invalid."
Assert-True ([string]$result.extensionVersion -ceq $ExpectedProductVersion) "Installed deadline extension version was invalid."
Assert-True (([System.IO.Path]::GetFullPath([string]$result.extensionPath)).Equals($installedPackageRoot, [System.StringComparison]::OrdinalIgnoreCase)) "Installed deadline extension path did not identify the installed VSIX package."
...
) "installed deadline product report"
Assert-True ([string]$report.schema -ceq "subversionr.release.m8-i6-installed-vsix-deadline.v1") "Installed deadline report schema was invalid."
Assert-True ([int]$report.schemaVersion -eq 1 -and [string]$report.kind -ceq "subversionr.installedSvnAnonymousDeadlineReport" -and [string]$report.scenario -ceq "deadline") "Installed deadline report identity was invalid."
Assert-ExactProperties $report.settlement @("code", "category", "messageKey", "retryable", "remoteFailure") "installed deadline settlement"
Assert-True (
...
) "Installed deadline settlement identity was invalid."
Assert-ExactProperties $report.settlement.remoteFailure @("category", "reason", "cleanupAppropriate") "installed deadline remote failure"
Assert-True (
...
) "Installed deadline remote failure was invalid."
Assert-ExactProperties $report.timing @("clock", "timeoutMs", "elapsedMs", "cleanupSlackMs") "installed deadline timing"
Assert-True (
...
) "Installed deadline timing proof was invalid."
Assert-True ($null -eq $report.diagnostics) "Installed deadline diagnostics must be null."
Assert-True ($report.nativeLaneReleased -eq $true -and $report.localSnapshotAfterTimeout -eq $true) "Installed deadline native-lane recovery proof was invalid."
Assert-ExactProperties $report.protocol @("major", "minor") "installed deadline protocol"
Assert-True ([int]$report.protocol.major -eq 1 -and [int]$report.protocol.minor -eq 35) "Installed deadline report protocol was invalid."
Assert-ExactProperties $report.trust @("acknowledgedEpoch", "consistent") "installed deadline trust"
Assert-True ($report.trust.consistent -eq $true -and [int]$report.trust.acknowledgedEpoch -ge 1) "Installed deadline report trust observation was invalid."
Assert-ExactProperties $report.authActivity @("credentialRequests", "credentialSettlements", "certificateRequests") "installed deadline authentication activity"
Assert-True ([int]$report.authActivity.credentialRequests -eq 0 -and [int]$report.authActivity.credentialSettlements -eq 0 -and [int]$report.authActivity.certificateRequests -eq 0) "Installed deadline anonymous execution produced authentication activity."
Assert-ExactProperties $report.repositorySession @("opened", "closed") "installed deadline repository session"
Assert-True ($report.repositorySession.opened -eq $true -and $report.repositorySession.closed -eq $true) "Installed deadline repository session did not close cleanly."
Assert-True ($report.diagnosticsRedacted -eq $true) "Installed deadline product report did not prove redacted execution."
Assert-ExactProperties $report.redaction @("rawUrls", "rawPaths", "rawContent") "installed deadline redaction"
Assert-True ($report.redaction.rawUrls -eq $false -and $report.redaction.rawPaths -eq $false -and $report.redaction.rawContent -eq $false) "Installed deadline report redaction contract was invalid."

$reportText = $report | ConvertTo-Json -Depth 32 -Compress
Assert-True (-not $reportText.Contains($RepositoryUrl) -and -not $reportText.Contains($workingCopyResolved)) "Installed deadline product report leaked fixture identity."
$temporaryRootsAfter = Get-TemporaryRootCount $remoteWorkersRoot
Assert-True ($temporaryRootsAfter -eq 0) "Installed deadline execution left operation temporary roots."
