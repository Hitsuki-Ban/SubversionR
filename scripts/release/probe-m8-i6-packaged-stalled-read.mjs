...
try {
  ...
  if (
    ...
    throw new Error("SUBVERSIONR_I6_PACKAGED_STALLED_READ_MODULE_INVALID");
  }
  if (!sameEndpoint(canonicalEndpointFromRepositoryUrl(options.repositoryUrl), endpoint)) {
    throw new Error("SUBVERSIONR_I6_PACKAGED_STALLED_READ_ORIGIN_INVALID");
  }
  ...
  connection = await startBackendProcess(
    {
      ...
      remoteStateRoot,
      clientName: "subversionr-m8-i6-packaged-stalled-read-evidence",
      clientVersion: "1",
      locale: "en",
    ...
    },
    {
      requestHandler: async (method) => {
        ...
        throw new Error("SUBVERSIONR_I6_PACKAGED_STALLED_READ_CREDENTIAL_HANDLER_FORBIDDEN");
      },
  ...
  if (temporaryRoots.length !== 0) {
    throw new Error("SUBVERSIONR_I6_PACKAGED_STALLED_READ_WORKER_TEMP_RESIDUE");
  }
...
function requireExpectedFailure(error) {
  if (
    ...
    throw new Error("SUBVERSIONR_I6_PACKAGED_STALLED_READ_ERROR_INVALID");
  }
  ...
  if (
    ...
    throw new Error("SUBVERSIONR_I6_PACKAGED_STALLED_READ_FAILURE_INVALID");
  }
}
...
function requireLocalSnapshot(value, session) {
  if (
    ...
    throw new Error("SUBVERSIONR_I6_PACKAGED_STALLED_READ_LOCAL_SNAPSHOT_INVALID");
  }
}
...
function requireDiagnosticsRedacted(value, options) {
  if (
    ...
    throw new Error("SUBVERSIONR_I6_PACKAGED_STALLED_READ_DIAGNOSTICS_INVALID");
  }
...
function requireRedacted(value, options) {
  ...
  for (const sensitive of [
    ...
    if (lowered.includes(sensitive.toLowerCase())) {
      throw new Error("SUBVERSIONR_I6_PACKAGED_STALLED_READ_DIAGNOSTICS_LEAK");
    }
  }
}
...
function requireCapabilities(activeConnection) {
  ...
  if (
    ...
    throw new Error("SUBVERSIONR_I6_PACKAGED_STALLED_READ_CAPABILITY_INVALID");
  }
...
function requireStableTrustAndNoCredentials(activeConnection, trustEpoch, inboundMethods) {
  if (!activeConnection.isRemoteSubmissionEnabled() || activeConnection.currentRemoteTrustEpoch() !== trustEpoch) {
    throw new Error("SUBVERSIONR_I6_PACKAGED_STALLED_READ_TRUST_INVALID");
  }
  if (inboundMethods.length !== 0) {
    throw new Error("SUBVERSIONR_I6_PACKAGED_STALLED_READ_CREDENTIAL_HANDLER_INVOKED");
  }
}
...
function requireOpenResponse(value, workingCopyPath, endpoint) {
  if (
    ...
    throw new Error("SUBVERSIONR_I6_PACKAGED_STALLED_READ_OPEN_INVALID");
  }
  let repositoryRoot;
  try {
...
function requireEnvelope(envelope, options, endpoint, trustEpoch) {
  if (
    ...
    Object.keys(envelope.profile).sort().join(",") !==
    ...
    envelope.profile.profileId !== "m8-i6-loopback-anonymous-stalled-read" ||
    ...
    throw new Error("SUBVERSIONR_I6_PACKAGED_STALLED_READ_ENVELOPE_INVALID");
  }
}
...
function requireCloseResponse(value, session) {
  if (
    ...
    throw new Error("SUBVERSIONR_I6_PACKAGED_STALLED_READ_CLOSE_INVALID");
  }
}
...
function parseRepositoryUrl(value) {
  ...
  try {
    ...
    throw new Error("SUBVERSIONR_I6_PACKAGED_STALLED_READ_URL_INVALID");
  }
  if (
    url.protocol !== "svn:" ||
...
function parseOptions(args) {
  ...
  if (args.length !== names.length * 2) {
    throw new Error("SUBVERSIONR_I6_PACKAGED_STALLED_READ_ARGUMENT_INVALID");
  }
  const parsed = {};
...
async function requireFilesystemContract(options) {
  ...
  if (!profile.isDirectory() || (await readdir(options.profileRoot)).length !== 0 || !workingCopy.isDirectory()) {
    throw new Error("SUBVERSIONR_I6_PACKAGED_STALLED_READ_FILESYSTEM_INVALID");
  }
}
...
async function requireWorkingCopyPreserved(workingCopyPath, expected) {
  ...
  if (
    ...
    throw new Error("SUBVERSIONR_I6_PACKAGED_STALLED_READ_WORKING_COPY_INVALID");
  }
}
...
async function captureUserContent(workingCopyPath, relativeDirectory, entries) {
  const directory = path.join(workingCopyPath, relativeDirectory);
...
async function requireFile(file) {
  if (!(await stat(file)).isFile()) {
    throw new Error("SUBVERSIONR_I6_PACKAGED_STALLED_READ_FILE_INVALID");
  }
}
...
function safeErrorCode(error) {
  ...
  return "SUBVERSIONR_I6_PACKAGED_STALLED_READ_PROBE_FAILED";
}
