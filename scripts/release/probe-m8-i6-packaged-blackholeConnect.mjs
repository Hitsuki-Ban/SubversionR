...
const STATE_OBSERVATION_TIMEOUT_MS = 30_000;
...
try {
  ...
  if (
    ...
    throw new Error("SUBVERSIONR_I6_PACKAGED_BLACKHOLE_CONNECT_MODULE_INVALID");
  }
  if (!sameEndpoint(canonicalEndpointFromRepositoryUrl(options.repositoryUrl), endpoint)) {
    throw new Error("SUBVERSIONR_I6_PACKAGED_BLACKHOLE_CONNECT_ORIGIN_INVALID");
  }

  const remoteStateRoot = path.join(options.profileRoot, "remote-state");
  await mkdir(remoteStateRoot);
  const inboundMethods = [];
  daemonStateObservation = createDaemonStateObservation(options.operationId);
  connection = await startBackendProcess(
    {
      ...
      remoteStateRoot,
      clientName: "subversionr-m8-i6-packaged-blackhole-connect-evidence",
      clientVersion: "1",
      locale: "en",
    ...
    },
    {
      requestHandler: async (method) => {
        ...
        throw new Error("SUBVERSIONR_I6_PACKAGED_BLACKHOLE_CONNECT_CREDENTIAL_HANDLER_FORBIDDEN");
      },
      notificationHandler: (method, params) => daemonStateObservation.onNotification(method, params),
    },
  );
  ...
  requireBlackholeConnectTiming(elapsedMs, options.timeoutMs);
  const daemonState = await daemonStateObservation.complete(opened);
  ...
  if (temporaryRoots.length !== 0) throw new Error("SUBVERSIONR_I6_PACKAGED_BLACKHOLE_CONNECT_WORKER_TEMP_RESIDUE");
  ...
  if (temporaryRootsAfterShutdown.length !== 0) {
    throw new Error("SUBVERSIONR_I6_PACKAGED_BLACKHOLE_CONNECT_WORKER_TEMP_RESIDUE");
  }
  await requireEmptyCheckoutJournal(remoteStateRoot);
  ...
  process.stdout.write(`${JSON.stringify({
    ...
    recovery: {
      required: false,
      daemonTerminalRecoveryFieldPresent: false,
      indeterminateObserved: false,
    },
    blackholeSettlement: {
      operationIdSha256: createHash("sha256").update(options.operationId, "utf8").digest("hex"),
  ...
  daemonStateObservation?.dispose();
...
function requireBlackholeConnectTiming(elapsedMs, timeoutMs) {
  if (
    ...
    throw new Error("SUBVERSIONR_I6_PACKAGED_BLACKHOLE_CONNECT_TIMING_INVALID");
  }
}
...
function requireExpectedFailure(error) {
  if (
    ...
    throw new Error("SUBVERSIONR_I6_PACKAGED_BLACKHOLE_CONNECT_ERROR_INVALID");
  }
  ...
  if (
    ...
    throw new Error("SUBVERSIONR_I6_PACKAGED_BLACKHOLE_CONNECT_FAILURE_INVALID");
  }
}

function createDaemonStateObservation(operationId) {
  const transitions = [];
  let terminal;
  let settled = false;
  let resolveObserved;
  ...
  const fail = (code) => {
    ...
    settled = true;
    clearTimeout(timer);
    rejectObserved(new Error(code));
  };
  const timer = setTimeout(
    () => fail("SUBVERSIONR_I6_PACKAGED_BLACKHOLE_CONNECT_STATE_TIMEOUT"),
    STATE_OBSERVATION_TIMEOUT_MS,
  );
  return {
    onNotification(method, params) {
      if (settled || method !== "remoteConnection/state" || !isRecord(params)) {
        fail("SUBVERSIONR_I6_PACKAGED_BLACKHOLE_CONNECT_STATE_INVALID");
        return;
      }
      if (
        Object.keys(params).sort().join(",") !== "epoch,repositoryId,state" ||
        typeof params.repositoryId !== "string" || !Number.isSafeInteger(params.epoch) || !isRecord(params.state)
      ) {
        fail("SUBVERSIONR_I6_PACKAGED_BLACKHOLE_CONNECT_STATE_INVALID");
        return;
      }
      ...
      if (
        transitions.length === 0 && Object.keys(state).sort().join(",") === "kind,operationId,startedAt" &&
        state.kind === "checking" && state.operationId === operationId &&
        typeof state.startedAt === "string" && !Number.isNaN(Date.parse(state.startedAt))
      ) {
        transitions.push({ repositoryId: params.repositoryId, epoch: params.epoch, kind: state.kind });
        return;
      }
      if (
        transitions.length === 1 && Object.keys(state).sort().join(",") === "kind,reason" &&
    ...
    async complete(session) {
      await observed;
      if (
        transitions.length !== 2 || terminal?.kind !== "unreachable" ||
        transitions.some((entry) => entry.repositoryId !== session.repositoryId || entry.epoch !== session.epoch)
      ) throw new Error("SUBVERSIONR_I6_PACKAGED_BLACKHOLE_CONNECT_STATE_INVALID");
      return {
        transitions: ["checking", "unreachable"],
        checkingOperationIdMatched: true,
        terminalKind: terminal.kind,
        terminalReason: terminal.reason,
        terminalRecoveryFieldPresent: Object.hasOwn(terminal, "recovery"),
...
function requireLocalSnapshot(value, session) {
  if (!isRecord(value) || value.repositoryId !== session.repositoryId || value.epoch !== session.epoch || value.source !== "libsvn-local") {
    throw new Error("SUBVERSIONR_I6_PACKAGED_BLACKHOLE_CONNECT_LOCAL_SNAPSHOT_INVALID");
  }
}
...
function requireDiagnosticsRedacted(value, options) {
  if (
    ...
    throw new Error("SUBVERSIONR_I6_PACKAGED_BLACKHOLE_CONNECT_DIAGNOSTICS_INVALID");
  }
...
function requireRedacted(value, options) {
  ...
  for (const sensitive of [
    ...
    if (lowered.includes(sensitive.toLowerCase())) throw new Error("SUBVERSIONR_I6_PACKAGED_BLACKHOLE_CONNECT_DIAGNOSTICS_LEAK");
  }
}
...
function requireCapabilities(activeConnection) {
  ...
  if (
    ...
    throw new Error("SUBVERSIONR_I6_PACKAGED_BLACKHOLE_CONNECT_CAPABILITY_INVALID");
  }
...
function requireStableTrustAndNoCredentials(activeConnection, trustEpoch, inboundMethods) {
  if (!activeConnection.isRemoteSubmissionEnabled() || activeConnection.currentRemoteTrustEpoch() !== trustEpoch) {
    throw new Error("SUBVERSIONR_I6_PACKAGED_BLACKHOLE_CONNECT_TRUST_INVALID");
  }
  if (inboundMethods.length !== 0) throw new Error("SUBVERSIONR_I6_PACKAGED_BLACKHOLE_CONNECT_CREDENTIAL_HANDLER_INVOKED");
}
...
function requireOpenResponse(value, workingCopyPath, endpoint) {
  if (
    ...
    throw new Error("SUBVERSIONR_I6_PACKAGED_BLACKHOLE_CONNECT_OPEN_INVALID");
  }
  let repositoryRoot;
  try {
...
function requireEnvelope(envelope, options, endpoint, trustEpoch) {
  if (
    ...
    envelope.profile.profileId !== "m8-i6-loopback-anonymous-blackhole-connect" ||
    ...
    throw new Error("SUBVERSIONR_I6_PACKAGED_BLACKHOLE_CONNECT_ENVELOPE_INVALID");
  }
}
...
function requireCloseResponse(value, session) {
  if (
    ...
    throw new Error("SUBVERSIONR_I6_PACKAGED_BLACKHOLE_CONNECT_CLOSE_INVALID");
  }
}
...
function parseRepositoryUrl(value) {
  ...
  try {
    ...
    throw new Error("SUBVERSIONR_I6_PACKAGED_BLACKHOLE_CONNECT_URL_INVALID");
  }
  if (
    url.protocol !== "svn:" ||
  ...
  if (!Number.isInteger(port) || port < 1 || port > 65_535) throw new Error("SUBVERSIONR_I6_PACKAGED_BLACKHOLE_CONNECT_URL_INVALID");
...
function parseOptions(args) {
  ...
  if (args.length !== names.length * 2) throw new Error("SUBVERSIONR_I6_PACKAGED_BLACKHOLE_CONNECT_ARGUMENT_INVALID");
  const parsed = {};
  for (let index = 0; index < args.length; index += 2) {
    const rawName = args[index];
    const rawValue = args[index + 1];
    if (typeof rawName !== "string" || !rawName.startsWith("--") || typeof rawValue !== "string") {
      throw new Error("SUBVERSIONR_I6_PACKAGED_BLACKHOLE_CONNECT_ARGUMENT_INVALID");
    }
    const name = rawName.slice(2);
    if (!names.includes(name) || name in parsed) throw new Error("SUBVERSIONR_I6_PACKAGED_BLACKHOLE_CONNECT_ARGUMENT_INVALID");
    parsed[name] = rawValue;
  }
  if (names.some((name) => !(name in parsed))) throw new Error("SUBVERSIONR_I6_PACKAGED_BLACKHOLE_CONNECT_ARGUMENT_INVALID");
  if (
...
async function requireFilesystemContract(options) {
  ...
  if (!profile.isDirectory() || (await readdir(options.profileRoot)).length !== 0 || !workingCopy.isDirectory()) {
    throw new Error("SUBVERSIONR_I6_PACKAGED_BLACKHOLE_CONNECT_FILESYSTEM_INVALID");
  }
}
...
async function requireWorkingCopyPreserved(workingCopyPath, expected) {
  ...
  if (
    ...
    throw new Error("SUBVERSIONR_I6_PACKAGED_BLACKHOLE_CONNECT_WORKING_COPY_INVALID");
  }
}
...
async function captureUserContent(workingCopyPath, relativeDirectory, entries) {
  ...
  for (const child of children) {
    ...
    if (!child.isFile()) throw new Error("SUBVERSIONR_I6_PACKAGED_BLACKHOLE_CONNECT_WORKING_COPY_INVALID");
...
async function requireFile(file) {
  if (!(await stat(file)).isFile()) throw new Error("SUBVERSIONR_I6_PACKAGED_BLACKHOLE_CONNECT_FILE_INVALID");
}
...
async function requireEmptyCheckoutJournal(remoteStateRoot) {
  try {
    await stat(path.join(remoteStateRoot, JOURNAL_TEMP_FILE));
    throw new Error("SUBVERSIONR_I6_PACKAGED_BLACKHOLE_CONNECT_JOURNAL_RESIDUE");
  } catch (error) {
    if (!isRecord(error) || error.code !== "ENOENT") throw error;
  }
  let serialized;
  try {
    serialized = await readFile(path.join(remoteStateRoot, JOURNAL_FILE), "utf8");
  } catch (error) {
  ...
  try {
    ...
    throw new Error("SUBVERSIONR_I6_PACKAGED_BLACKHOLE_CONNECT_JOURNAL_INVALID");
  }
  if (
    !isRecord(journal) || Object.keys(journal).sort().join(",") !== "entries,schemaVersion" ||
    journal.schemaVersion !== 1 || !Array.isArray(journal.entries) || journal.entries.length !== 0
  ) throw new Error("SUBVERSIONR_I6_PACKAGED_BLACKHOLE_CONNECT_JOURNAL_INVALID");
}
...
function requireAbsolute(value) {
  if (!path.isAbsolute(value)) throw new Error("SUBVERSIONR_I6_PACKAGED_BLACKHOLE_CONNECT_ARGUMENT_INVALID");
...
function safeErrorCode(error) {
  ...
  return "SUBVERSIONR_I6_PACKAGED_BLACKHOLE_CONNECT_PROBE_FAILED";
}
