...
describe("installed SVN anonymous recovery-blocked report", () => {
  ...
  it("restores the block after restart, rejects a same-target retry locally, confirms exactly, and checks out from healthy svnserve", async () => {
    let listCalls = 0;
    const sendRequest = vi.fn(async (method: string, params: unknown): Promise<unknown> => {
      if (method === "remote/listCheckoutTargetRecoveries") {
        listCalls += 1;
        return { entries: listCalls <= 2 ? [blockedEntry()] : [] };
      }
      if (method === "repository/checkout" && listCalls === 1) {
      ...
      expect(method).toBe("repository/checkout");
  ...
  it("fails closed when the blocked retry creates a target before confirmation", async () => {
    const sendRequest = vi.fn(async (method: string): Promise<unknown> => {
      ...
      if (method === "repository/checkout") throw localBlockedError();
