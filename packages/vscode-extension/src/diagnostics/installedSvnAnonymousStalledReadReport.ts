...
const REPORT_SCHEMA = "subversionr.release.m8-i6-installed-svn-anonymous-stalled-read.v1";
const REPORT_KIND = "subversionr.installedSvnAnonymousStalledReadReport";
const MAX_REDACTION_VALUE_BYTES = 32_768;

type StalledReadConnection = Pick<
  BackendConnection,
  "initializeResult" | "isRemoteSubmissionEnabled" | "currentRemoteTrustEpoch" | "sendRequest"
>;

export interface InstalledSvnAnonymousStalledReadReportOptions {
  expectedToken: string | undefined;
  request: unknown;
  initialize(): Promise<StalledReadConnection>;
...
interface InstalledSvnAnonymousStalledReadRequest {
  token: string;
...
export async function collectInstalledSvnAnonymousStalledReadReport(
  options: InstalledSvnAnonymousStalledReadReportOptions,
  ...
  if (
    ...
    throw reportError("SUBVERSIONR_INSTALLED_SVN_ANONYMOUS_STALLED_READ_ENVELOPE_INVALID");
  }
  ...
  try {
    ...
    if (observedError === undefined) {
      throw reportError("SUBVERSIONR_INSTALLED_SVN_ANONYMOUS_STALLED_READ_UNEXPECTED_SUCCESS");
    }
    ...
    if (
      ...
      throw reportError("SUBVERSIONR_INSTALLED_SVN_ANONYMOUS_STALLED_READ_LOCAL_SNAPSHOT_INVALID");
    }
    ...
    try {
      ...
      throw reportError("SUBVERSIONR_INSTALLED_SVN_ANONYMOUS_STALLED_READ_DIAGNOSTICS_INVALID");
    }
  ...
  if (
    ...
    throw reportError("SUBVERSIONR_INSTALLED_SVN_ANONYMOUS_STALLED_READ_AUTH_ACTIVITY_INVALID");
  }
  if (!expectedFailureObserved || !localSnapshotAfterTimeout) {
    throw reportError("SUBVERSIONR_INSTALLED_SVN_ANONYMOUS_STALLED_READ_SETTLEMENT_INVALID");
  }
...
function parseRequest(
  value: unknown,
  expectedToken: string | undefined,
): InstalledSvnAnonymousStalledReadRequest {
  if (typeof expectedToken !== "string" || expectedToken.length === 0 || !isRecord(value)) {
    throw reportError("SUBVERSIONR_INSTALLED_SVN_ANONYMOUS_STALLED_READ_FORBIDDEN");
  }
  ...
  if (
    ...
    throw reportError("SUBVERSIONR_INSTALLED_SVN_ANONYMOUS_STALLED_READ_REQUEST_INVALID");
  }
  return {
    token: value.token,
...
function requireLoopbackSvnOrigin(repositoryUrl: string): CanonicalEndpoint {
  ...
  try {
    ...
    throw reportError("SUBVERSIONR_INSTALLED_SVN_ANONYMOUS_STALLED_READ_ORIGIN_INVALID");
  }
  if (
    endpoint.scheme !== "svn" ||
...
function requireCandidateCapabilities(connection: StalledReadConnection): number {
  ...
  if (
    ...
    throw reportError("SUBVERSIONR_INSTALLED_SVN_ANONYMOUS_STALLED_READ_CAPABILITY_UNAVAILABLE");
  }
  ...
  if (
    ...
    throw reportError("SUBVERSIONR_INSTALLED_SVN_ANONYMOUS_STALLED_READ_TRUST_EPOCH_INVALID");
  }
  return trustEpoch;
}
...
function requireOpenedSession(
  ...
  if (
    ...
    throw reportError("SUBVERSIONR_INSTALLED_SVN_ANONYMOUS_STALLED_READ_OPEN_INVALID");
  }
...
function requireCurrentRedactedDiagnostics(
  value: unknown,
  request: InstalledSvnAnonymousStalledReadRequest,
): void {
...
function requireRedacted(value: unknown, request: InstalledSvnAnonymousStalledReadRequest): void {
  ...
  if (sensitive.some((entry) => normalized.includes(entry))) {
    throw reportError("SUBVERSIONR_INSTALLED_SVN_ANONYMOUS_STALLED_READ_DIAGNOSTICS_LEAK");
  }
}
...
function requireStableTrust(connection: StalledReadConnection, trustEpoch: number): void {
...
export class InstalledSvnAnonymousStalledReadReportError extends Error {
  public readonly messageKey = "error.diagnostics.installedSvnAnonymousStalledReadReportInvalid";

  public constructor(public readonly code: string) {
    super(code);
    this.name = "InstalledSvnAnonymousStalledReadReportError";
  }
}

function reportError(code: string): InstalledSvnAnonymousStalledReadReportError {
  return new InstalledSvnAnonymousStalledReadReportError(code);
}
