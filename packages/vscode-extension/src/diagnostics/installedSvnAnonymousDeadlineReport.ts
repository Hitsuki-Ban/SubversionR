...
const REPORT_SCHEMA = "subversionr.release.m8-i6-installed-vsix-deadline.v1";
const REPORT_KIND = "subversionr.installedSvnAnonymousDeadlineReport";
const CLEANUP_SLACK_MS = 5_000;
const MAX_REDACTION_VALUE_BYTES = 32_768;

type DeadlineConnection = Pick<
  BackendConnection,
  "initializeResult" | "isRemoteSubmissionEnabled" | "currentRemoteTrustEpoch" | "sendRequest"
>;

export interface InstalledSvnAnonymousDeadlineReportOptions {
  expectedToken: string | undefined;
  request: unknown;
  initialize(): Promise<DeadlineConnection>;
...
interface InstalledSvnAnonymousDeadlineRequest {
  token: string;
...
export async function collectInstalledSvnAnonymousDeadlineReport(
  options: InstalledSvnAnonymousDeadlineReportOptions,
  ...
  if (
    ...
    throw reportError("SUBVERSIONR_INSTALLED_SVN_ANONYMOUS_DEADLINE_ENVELOPE_INVALID");
  }
  ...
  try {
    ...
    requireDeadlineTiming(elapsedMs, request.timeoutMs);
    if (observedError === undefined) {
      throw reportError("SUBVERSIONR_INSTALLED_SVN_ANONYMOUS_DEADLINE_UNEXPECTED_SUCCESS");
    }
    ...
    if (
      ...
      throw reportError("SUBVERSIONR_INSTALLED_SVN_ANONYMOUS_DEADLINE_LOCAL_SNAPSHOT_INVALID");
    }
    ...
    try {
      ...
      throw reportError("SUBVERSIONR_INSTALLED_SVN_ANONYMOUS_DEADLINE_DIAGNOSTICS_INVALID");
    }
  ...
  if (
    ...
    throw reportError("SUBVERSIONR_INSTALLED_SVN_ANONYMOUS_DEADLINE_AUTH_ACTIVITY_INVALID");
  }
  if (!expectedFailureObserved || !localSnapshotAfterTimeout || elapsedMs === undefined) {
    throw reportError("SUBVERSIONR_INSTALLED_SVN_ANONYMOUS_DEADLINE_SETTLEMENT_INVALID");
  }
...
function parseRequest(
  value: unknown,
  expectedToken: string | undefined,
): InstalledSvnAnonymousDeadlineRequest {
  if (typeof expectedToken !== "string" || expectedToken.length === 0 || !isRecord(value)) {
    throw reportError("SUBVERSIONR_INSTALLED_SVN_ANONYMOUS_DEADLINE_FORBIDDEN");
  }
  ...
  if (
    ...
    throw reportError("SUBVERSIONR_INSTALLED_SVN_ANONYMOUS_DEADLINE_REQUEST_INVALID");
  }
  return {
    token: value.token,
...
function requireLoopbackSvnOrigin(repositoryUrl: string): CanonicalEndpoint {
  ...
  try {
    ...
    throw reportError("SUBVERSIONR_INSTALLED_SVN_ANONYMOUS_DEADLINE_ORIGIN_INVALID");
  }
  if (
    endpoint.scheme !== "svn" ||
...
function requireCandidateCapabilities(connection: DeadlineConnection): number {
  ...
  if (
    ...
    throw reportError("SUBVERSIONR_INSTALLED_SVN_ANONYMOUS_DEADLINE_CAPABILITY_UNAVAILABLE");
  }
  ...
  if (
    ...
    throw reportError("SUBVERSIONR_INSTALLED_SVN_ANONYMOUS_DEADLINE_TRUST_EPOCH_INVALID");
  }
  return trustEpoch;
}
...
function requireOpenedSession(
  ...
  if (
    ...
    throw reportError("SUBVERSIONR_INSTALLED_SVN_ANONYMOUS_DEADLINE_OPEN_INVALID");
  }
...
function requireExpectedFailure(error: unknown): void {
  if (
...
function requireDeadlineTiming(elapsedMs: number, timeoutMs: number): void {
  if (
    !Number.isFinite(elapsedMs) ||
    elapsedMs < timeoutMs ||
    elapsedMs > timeoutMs + CLEANUP_SLACK_MS
  ) {
    throw reportError("SUBVERSIONR_INSTALLED_SVN_ANONYMOUS_DEADLINE_TIMING_INVALID");
  }
}
...
function requireCurrentRedactedDiagnostics(
  value: unknown,
  request: InstalledSvnAnonymousDeadlineRequest,
): void {
...
function requireRedacted(value: unknown, request: InstalledSvnAnonymousDeadlineRequest): void {
  ...
  if (sensitive.some((entry) => normalized.includes(entry))) {
    throw reportError("SUBVERSIONR_INSTALLED_SVN_ANONYMOUS_DEADLINE_DIAGNOSTICS_LEAK");
  }
}
...
function requireStableTrust(connection: DeadlineConnection, trustEpoch: number): void {
...
export class InstalledSvnAnonymousDeadlineReportError extends Error {
  public readonly messageKey = "error.diagnostics.installedSvnAnonymousDeadlineReportInvalid";

  public constructor(public readonly code: string) {
    super(code);
    this.name = "InstalledSvnAnonymousDeadlineReportError";
  }
}

function reportError(code: string): InstalledSvnAnonymousDeadlineReportError {
  return new InstalledSvnAnonymousDeadlineReportError(code);
}
